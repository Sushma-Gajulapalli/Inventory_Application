...
//		if ($currblk['inner_width'] < $mw) { throw new MpdfException("DIV is too narrow for text to fit!"); }
			...
			case 'TABLE': // TABLE-BEGIN
				...
				if (isset($properties['BACKGROUND-COLOR']) && is_array($properties['BACKGROUND-COLOR'])) {
					$table['bgcolor'][-1] = $properties['BACKGROUND-COLOR'];
				} else if (isset($properties['BACKGROUND']) && is_array($properties['BACKGROUND-COLOR'])) {
				...
				if ($w) {
					...
					if (strpos($w, '%') && $this->mpdf->tableLevel > 1 && !$this->mpdf->ignore_table_percents && $this->mpdf->keep_table_proportions) {
						$table['wpercent'] = (int)$w + 0;  // makes 80% -> 80
					}
			...
			case 'TR':
				...
				if (isset($properties['BACKGROUND-COLOR'])) {
					if(key((array)$this->mpdf->tableLevel) == 0 || key((array)$this->mpdf->tableLevel) == 0 || key((array)$this->mpdf->row) == 0){
						  $properties['BACKGROUND-COLOR'] = $properties['BACKGROUND-COLOR'];

					}else{
					 $this->mpdf->table[key((array)$this->mpdf->tableLevel)][current((array)$this->mpdf->tbctr[key((array)$this->mpdf->tableLevel)])]['bgcolor'][key((array)$this->mpdf->row)] = $properties['BACKGROUND-COLOR'];
					}
